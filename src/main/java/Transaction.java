...
public class Transaction {
    ...
    public boolean processTransaction() {
        ...
        for(TransactionInput i : inputs) {
            i.UTXO = VQD_Chain.UTXOs.get(i.transactionOutputId);
        }

        //Kiểm tra giao dịch hợp lệ
        if(getInputsValue() < VQD_Chain.minimumTransaction) {
            System.out.println("Dau vao cua giao dich qua be: " + getInputsValue());
            System.out.println("Vui long nhap so tien giao dich lon hon" + VQD_Chain.minimumTransaction);
            return false;
        }
        ...
        for(TransactionInput i : inputs) {
            ...
            VQD_Chain.UTXOs.remove(i.UTXO.id);
        }
